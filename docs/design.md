...
│       ├── extract_manifest.md      # Declared package names and path aliases of manifests (ExtractFacts)
...
- `build_facts_focus_note(lines, limit=FACTS_FOCUS_LIMIT) -> str` (ExtractFacts follow-up): the `{focus_note}` listing `number: text` of the uncovered lines, at most `FACTS_FOCUS_LIMIT` (60) with a count of the rest; a line longer than 200 characters is cut with `...` (the model quotes from the full source)
- `parse_file_indices(value) -> list[int]` (DeterministicFileMapper): strict — an int, an annotated number (`"3 # path"`) or an ascending range (`"0-3"`, `"4..7"`, expanded); a negative number, a path with digits (`src/v2/api.py`) or prose names no index. `parse_file_index` (first digit run) stays for the tutorial-mode index lists
...
- Also lists the **manifests** among the other files: files that declare a package, module, crate or workspace name or import path aliases for the project's own code (examples in the prompt: package.json, tsconfig.json, go.mod, Cargo.toml, pyproject.toml, setup.cfg, composer.json, pom.xml, build.gradle, *.csproj, mix.exs, pubspec.yaml, *.gemspec — a hint, not a gate)
- Returns YAML `{code: [indices], manifests: [indices]}`; a bare list (older reply shape) is the code list
...
Language-agnostic checks of the claims `ExtractFacts` gets from `extract_facts.md`. Nothing parses a programming language: every claim quotes the source, and a claim is kept only when code finds the quote in that file. Line numbers always come from the match, never from the LLM. Verification proves presence, not completeness. Measured offline on a fixture of 15 projects — Python, Go, TypeScript, Java, C#, C++, Ruby, Rust, PHP, JavaScript, plus npm-workspace, tsconfig-paths, go.mod, Cargo-workspace and Python-monorepo projects whose imports go through manifest-declared names (`.agents/work/facts_fixture`, `.agents/work/facts_eval.py`, gitignored; 85 hand-labelled edges, independently re-labelled): ideal claims give 85/85 edges and no false one; 72/75 adversarial dependency claims, 15/16 symbol claims, 53/57 manifest-resolution claims and 19/21 manifest claims are judged as labelled (the rest deliberate: an ambiguous usage line rejected while its import line proves the edge; paraphrased targets kept only because the line uses names only that module defines, so the edge is still true; workspace member paths not treated as importable names; a member claimed without its parent kept by its unique full signature). With the real model (`.agents/work/facts_live_eval.py`, gemini-3.8-flash at medium effort, raw replies kept under `.agents/work/facts_live/` and replayable offline): 147/147 edges (85 on the 15 fixture projects, 62 on this repo), no false edge; every true symbol found (187 in the fixture, 326 in this repo) and every claim's quote found in its file; after the follow-ups every import and declaration line of this repo is reported (713/713; 8 follow-up calls in 7 of its 20 modules, two rounds for the 1,500-line `utils/facts.py`), and 2 fixture lines were declined (C# top-level statements); $1.22 for the five evaluation runs together.
...
class ModuleTable:            # documented modules: paths, stems, directories, owned names, contents, manifest rules
...
def judge_dependency(claim: dict, source_path: str, source_words: set, table: ModuleTable, relative_style: bool, shadowed: frozenset = frozenset()) -> tuple[str, list]
...
- **Duplicates:** one fact quoted twice (a whole signature and its first line, an import and its block) is kept once — symbols by parent, name and line, dependencies by target and line, config / errors by name and line — and counted as claimed once. `merge_claims` (first pass + follow-up) drops a restated claim by its identity fields (symbol parent, name, signature; dependency target, evidence; config / error name, evidence), whitespace collapsed, keeping the first
...
- **Completeness hints** (`completeness_hints`, `code_view`, `enclosing_blocks`, `declaration_like`, `import_statement_like`, `import_block_lines`): first `code_view` blanks block comments and block strings (`/* … */`, `"""` / `'''`, replaced by `""` so `HELP = """…` stays an assignment) and trailing `#` / `//` / `-- ` comments after whitespace; delimiters inside a one-line string literal (`'"""'`, `"src/*"`), a `/*` right after a name (a glob: `lib/*.sh`) and an opener whose closer never follows in the file do not count. Then lines that look like imports (an import word first — after `#` for `#include`, after `@annotations` — followed by a path, name or quoted specifier, not by an operator, call, assignment or lone block opener: `source = x`, `open(path)`, `using (var …)`, `import {`, `load().then(` and prose are excluded; `require('x')` / `import('x')` calls anywhere, and Lua / Ruby's `require "x"`; a shell `. path`; not a sentence (`from here on, …`, `source code is read once.`); `export` / `part` with a quoted specifier; Go import-block path lines) or declarations (after `@annotations` and modifiers — incl. `private(set)`, `lateinit`, `implicit`, `case class` — a declaration word followed by a name or a Go receiver, or Elixir's `defstruct [...]`; unindented `const` / `let` / `var` …; `UPPER_CASE =` constants whose value is not prose, also after modifiers (`readonly CONFIG_DIR=…`, `export API_URL=…`); unindented typed values after modifiers (`final String apiUrl = 'x';`); unindented C-like `type name(...) {`, also with `const` / `noexcept` / `async` before the brace; unindented `name :: type` signatures; SQL `CREATE TABLE / VIEW / FUNCTION …`; `module.exports = function`; `[type] name(` heads with a modifier, or ending in `{` when the parentheses hold no `=>`, string or call (so `describe('x', () => {`, `useEffect(() => {` and match arms are not heads); `#define`). A declaration inside a function body is local and skipped (`enclosing_blocks`: one pass with a stack of open blocks by indentation; a line opens a `function` — a function word, a method head, a closure or callback such as `describe('x', () => {`, `do |x|` —, a `container` — class, struct, interface, impl, module, namespace … —, a `control` block, looked through, or `other`; bracket-only and closing lines such as `) -> None:`, `} else {`, `end` open nothing). Class properties and fields are not hinted: the prompt asks for module-level constants and exported variables, not members other than methods. Hint vocabularies like `_IMPORT_WORDS`: they only decide what a follow-up asks about, never a fact. Import lines are covered by dependency claims — their matched lines and every line with the same text as a match or quote (one import repeated inside several functions is one fact); declaration lines by the lines symbol claims were located on (`ambiguous_lines` too: `def run(self)` in two classes is two symbols; a config fact on a declaration does not report its symbol). Measured without an LLM (`.agents/work/facts_hint_eval.py`, 96 files: the fixture and this repo): of claims deliberately dropped from the ideal replies, 97.1% sit on an uncovered line and an ideal follow-up recovers 98.6% (the rest: inline qualified uses and `throw new X` lines that are not import lines, and repeated in-function imports whose edge another quote already proves); with the full ideal replies 12 lines stay uncovered (constructors the fixture omits, C# top-level statements). On 216 labelled lines in 18 languages and formats (Kotlin, Swift, Scala, Elixir, Haskell, Lua, Dart, shell, SQL, YAML, Vue, Jupyter, minified JS …), 9 false hints and 7 misses, all misses class properties or a constructor
- **Unexplained names** (`unexplained_names`): names that exactly one other module defines, used in the source with no verified dependency on that module — a recall hint for same-package use without an import (recorded per module; can be a word in a comment or string)
- **Target written in its evidence** (`target_written`, in `judge_dependency`): the claimed target must be written in its own line, judged by the line's shape. A line with quoted specifiers (JS/TS, Go, C includes, PHP, Terraform …): the target equals one of them (`./`, `/`, leading dots ignored) or is the end of a path built at run time (`"$(dirname "$0")/../lib/log.sh"`, `__DIR__ . '/src/Db.php'`) — so a truncated `@acme/api` of `'@acme/api/src/client'` or a rewritten `src/lib/money` for `'@lib/money'` is not written. A line without quotes (Python, Java, Rust, C# …): the target's words occur in order as whole words (`app.helpers` in `from app import helpers`, `.util` in `from . import util`; not `shop::store` in `use shop_core::store::Repo`). A target that is not written is a paraphrase, kept only when the model named the module's file and the line passes the evidence rule for it, or the line uses a name only that module defines. In the live evaluation every one of 209 real dependency claims wrote its target in its line
- **Bare names shadowed by an external import** (`shadowed_names`): a bare target that is the last segment of an external import and of no internal import of the same source (`store` of `example.com/other/store`) is external
- **Manifests** (`verify_manifest_claims`, `manifest_rules`, `ModuleTable.resolve_manifest`): a `provides` name counts when its quoted line contains it verbatim as the value of a naming key (`_NAME_KEYS`: `name`, `module`, `package`, `artifactId`, `app` …) — a name written as a dependency key or list item (`"@acme/api": "^1.2.0"`, workspace member paths) is rejected; a declared `entry` counts only as the value of an entry key (`_ENTRY_KEYS`: `main`, `exports`, `types`, `bin`, `path` …), not inside a script line, else it is dropped; names with `*` are ignored; an alias when the line contains the alias and target. Resolution runs before the relative-import convention: an alias rewrites the prefix into its root-relative template (`@lib/*` → `src/lib/*`, `#db/*` → `src/db/*.ts`); a package name matches the target's leading segments, case-folded with `-` as `_` (`shop_core::store` for `shop-core`, `example.com/shop/internal/store` for the Go module `example.com/shop`): no rest → the declared entry, else an index file of the package directory (or its `src`, `lib`, name directory), else its files whose names the source uses; a rest → a file or directory under the package ending with it, else (an item imported through the package) the package root, like a barrel. A multi-part target sharing a declared name's first segment without matching it (`example.com/other/store`, `@acme/unknown`), or, once manifests are known, any sigil-led name (`@scope/x`, `#x`) nothing declares, is external. The evidence must import the target (`_imports_target`: an import-like line, a Go import-block path, the target as a quoted specifier, or an inline qualified use in code) and write it — a comment, log text or string constant naming a declared package never counts. Via `manifest`
...
- **`facts.json`** (`facts_path`): `docs/api/facts.json` under `--mkdocs` (published with the site; CI's `docs/api/` cache keeps it), else next to the pages. Document: `schema` (1), `project`, `commit` (`git -C <crawled dir> rev-parse HEAD`, not this tool's checkout; `null` for `--repo`), `note` (presence verified, completeness not), `modules`: per path `source_sha256`, `lines`, `symbols` (`name`, `kind`, `parent`, `visibility`, `line`, `signature`, `match`, optional `ambiguous_lines` / `truncated`), `depends_on` (`module`, `target` as written, `line`, `evidence`, `via`: `path` / `package` / `symbol` / `namespace` / `manifest`), `used_by`, `external`, `config`, `errors`, `coverage` (`claimed` / `verified`; items the parser could not read count as claimed), `rejected` (with reasons), `missed_imports`, `unexplained_names`, `completeness` (`import_lines`, `declaration_lines` — counts —, `unreported_lines` — uncovered lines no follow-up asked about —, `declined_lines` — uncovered lines a follow-up asked about, judged not facts by the model —, `followup`: `status` `none` / `done` / `failed` and `asked`, the line numbers it listed), `facts_hash`, `claims` (the raw reply, merged with the follow-up, re-verified on every run); and `manifests`: per manifest path `provides` (`name`, `entry`, `line`, `evidence`), `aliases` (`alias`, `target`, `base`, `line`, `evidence`), `rejected`, `claimed`, `found`, `facts_hash` (`null` when reply items stayed unparsed: asked again next run), `claims`. Written atomically (`save_facts`)
...
- **Writes:** `shared["abstractions"]`, `shared["chapter_order"]` (sorted by directory depth), `shared["relationships"]`, `shared["manifest_files"]` (manifest indices that are not code and not empty; read by ExtractFacts, never documented)
- **exec return:** `(code_indices, manifest_indices)` (`parse_file_indices` on every entry: strict, ranges expanded, anything else dropped)
...
| `focus_note` | `""` on the first pass; on the follow-up, `build_facts_focus_note(uncovered lines)` (at most `FACTS_FOCUS_LIMIT` lines) |

Manifest items use `prompts/common/extract_manifest.md` (`project_name`, `file_path`, `source`).

- **prep:** one item per `shared["abstractions"]` module (api-reference: one file each) with `path`, `content`, `facts_hash`, `cached_claims` (from the previous `facts.json` when the hash matches, under `--incremental` or with the LLM cache on; else `None`), `cached_followup` (that module's stored `completeness.followup`), the template, `use_cache`, `thinking_level` (`resolve_thinking_level(shared, "extract_facts")`), `max_tokens`. Reading the previous facts.json with the LLM cache on also keeps a retried module's accepted reply, which the LLM cache (first attempt only) does not hold
- **exec:** cached claims → `FACTS_CACHE_HIT`, no call — unless the stored follow-up `failed`, which is asked again on the cached claims. Otherwise `LLM_CALL_EXTRACT_FACTS`, `log_token_estimation`, `call_llm(prompt, use_cache=(use_cache and self.cur_retry == 0), thinking_level=..., step="extract_facts")`, `parse_facts_response`, `verify_file_claims`. Each attempt's result is remembered per path (`self.best`, reset on attempt 0); with ≥ 4 claims and fewer than half found (a paraphrasing reply) it emits `FACTS_LOW_COVERAGE` and raises to retry uncached — except on the last attempt. It returns the **best** attempt's claims (most found), not the last
- **Follow-up (completeness, `_follow_up`):** after the chosen attempt, `completeness_hints` lists the import-like and declaration-like lines no claim of the matching kind covers. While some were never asked, another call (`FACTS_FOLLOWUP`, same template with `focus_note` listing them, at most `FACTS_FOCUS_LIMIT` = 60 per call) asks for the facts on exactly those lines, up to `ExtractFacts.FOLLOWUP_ROUNDS` (3) calls, so a large file the first pass covered thinly is asked in chunks; `use_cache=(use_cache and self.cur_retry == 0)`, and a reply that raises or does not parse gets one uncached retry. Each answer is merged (`merge_claims`) and verified like the first. Result `followup`: `{"status": "none", "asked": []}` (nothing uncovered, no call), `{"status": "done", "asked": [every listed line number]}` (lines past the rounds stay `unreported_lines`), or `{"status": "failed", "asked": [lines of the rounds that succeeded]}` (the claims so far stand; the next run resumes from the cached claims, skipping `asked`). A cached `done` or `none` is final
- **Manifest items** (`kind: "manifest"`, one per `shared["manifest_files"]`, `_exec_manifest`): `LLM_CALL_EXTRACT_MANIFEST`, `parse_facts_response(response, MANIFEST_KEYS)`, cached by their own `facts_hash` in facts.json `manifests`; no follow-up. A reply with unparsed items raises to retry uncached; on the last attempt its readable items are kept with `facts_hash: None`, so the next run asks again
- **exec_fallback:** `WARN_FACTS_FALLBACK`; the best earlier attempt if one parsed (follow-up `failed`: none ran), else `{"claims": None, "facts_hash": None}`: no facts for that module this run, retried next run
- **post:** verifies the manifests (`verify_manifest_claims`), re-verifies every module's claims, builds the `ModuleTable` (with the verified manifests), `verify_dependencies`, fills `used_by`, writes `facts.json` (`FILE_WROTE`), sets `shared["module_facts"]`, emits `FACTS_MISSED_IMPORTS` (debug), `DONE_FACTS` (with the count of modules without facts), `FACTS_COMPLETENESS` (reported / total import and declaration lines, lines declined after a follow-up, lines never asked about, follow-up calls made this run incl. retries, modules whose follow-up failed) and the `extract_facts` step subtotal. Per module it records `completeness` (`import_lines`, `declaration_lines`, `unreported_lines` — uncovered and not asked —, `declined_lines` — uncovered after the follow-up asked —, `followup`) and `unexplained_names`
**`post()` return:** `"default"`
...
|---|---|---|---|
| DeterministicFileMapper | dict | `code[]`, `manifests[]` (int lists) | Code files to document and manifests for ExtractFacts; a bare int list is the code list; uses `build_code_file_filter_prompt` |
| ExtractFacts (manifest) | dict | `provides[].name`, `.entry`, `.evidence`; `aliases[].alias`, `.target`, `.base`, `.evidence` | `parse_facts_response(response, MANIFEST_KEYS)`, checked by `verify_manifest_claims` |
...
Shared prompts that are NOT mode-specific. `group_modules.md` and `translate_strings.md` are loaded directly by path; `extract_facts.md` and `extract_manifest.md` via `load_prompt_template(..., mode="common")`.
...
| `{focus_note}` | `""`, or `build_facts_focus_note` on the follow-up | The uncovered lines to report (placed just before the YAML instruction) |

The YAML example contains a literal `{{token}}` (escaped brace).

#### `extract_manifest.md` — Declared Package Names and Path Aliases (ExtractFacts)
One call per manifest (`shared["manifest_files"]`). The reply lists `provides` (every name under which the file makes its own folder's code importable: package / module / crate / workspace-member name, with an optional declared `entry` file; not its dependencies) and `aliases` (import path aliases for the project's own code: `alias`, `target`, optional `base` such as tsconfig `baseUrl`). Every evidence is a verbatim `|-` quote containing the name (or the alias and target).

| Variable | Source | Description |
|---|---|---|
| `{project_name}` | `shared["project_name"]` | Project display name |
| `{file_path}` | manifest path | The manifest the names come from |
| `{source}` | file content | The whole manifest |
