...
PAGE SKELETON (MANDATORY): Every chapter MUST follow this section ordering. You may SKIP a section if the component has no relevant content for it, but you MUST NOT invent new `##` headings or rename these. ALL other content belongs inside these sections as `###`/`####` sub-sections or prose paragraphs.
