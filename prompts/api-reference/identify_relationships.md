...
1. A high-level technical `summary` of the project's API architecture in a few sentences{lang_hint}. Use markdown formatting with **bold** and *italic* text to highlight critical architectural components and technologies.
2. A list (`relationships`) describing the key technical interactions, dependencies, or data flows between these modules. For each relationship, specify:
    ...
    - `label`: A precise technical label for the interaction **in just a few words**{lang_hint}.
      ...
      Examples of bad labels: "uses", "manages", "depends on", "related to" (too vague to be useful for architecture understanding)
