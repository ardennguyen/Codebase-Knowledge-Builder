...
- Link to other modules using Markdown links with relative paths. Each module's doc path is shown in the Index above as `(doc: path.md)`. The link target filename MUST be copied EXACTLY and VERBATIM from the `(doc: ...)` annotation — NEVER re-derive, re-slugify, or guess filenames. Compute the relative path from your location ({current_doc_path}) to the target{link_lang_note}. Translate the surrounding prose text, not the code.
