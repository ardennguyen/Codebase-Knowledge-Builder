...
4. Generate a complete codebase tutorial by running the main script:
    ...
    - `--mkdocs` - Format output for MkDocs Material (adds YAML frontmatter & nav snippet).
      ...
      - `api-reference` mode also writes `facts.json` (published at `api/facts.json`): every module's symbols, dependencies, config keys and errors, each verified against a quoted line of the source by language-agnostic checks, with line numbers. The module dependency graph is drawn from these verified edges, and other tools or AI agents can read the file as ground truth.
...
4. Tạo một bản hướng dẫn toàn diện về mã nguồn bằng cách chạy tập lệnh chính:
    ...
    - `--mkdocs` - Định dạng đầu ra cho MkDocs Material (thêm YAML frontmatter & nav snippet).
      ...
      - Chế độ `api-reference` còn ghi `facts.json` (xuất bản tại `api/facts.json`): ký hiệu, phụ thuộc, khóa cấu hình và lỗi của từng module, mỗi mục đều được đối chiếu với một dòng trích từ mã nguồn bằng cách kiểm tra không phụ thuộc ngôn ngữ, kèm số dòng. Đồ thị phụ thuộc module được vẽ từ các cạnh đã kiểm chứng này, và các công cụ khác hoặc AI agent có thể đọc tệp này như nguồn sự thật.
