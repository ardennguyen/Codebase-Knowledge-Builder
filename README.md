...
3. Set up LLM by copying `.env.sample` to `.env` and providing credentials. By default, you can use the AI Studio key for Gemini by setting the `GEMINI_API_KEY` environment variable (or `GEMINI_PROJECT_ID` for Vertex AI). If you want to use another LLM, you can set the `LLM_PROVIDER` environment variable (e.g. `OPENROUTER`), and then set the model, url, and API key (e.g. `OPENROUTER_MODEL`, `OPENROUTER_BASE_URL`,`OPENROUTER_API_KEY`). If using Ollama, set `LLM_PROVIDER=OLLAMA` and the base url (e.g. `OLLAMA_BASE_URL=http://localhost:11434`) — the API key can be omitted.
   ...
   **Thinking on every provider:** `--thinking-profile` (default `auto` = `balanced` on Anthropic, Gemini and OpenRouter) is mapped per model:
   ...
   Every LLM call prints a usage line (billed input vs the estimate, output with thinking, cached tokens, cost), batch steps print subtotals, and every run ends with a token/cost summary per provider and per step (cost estimated from list prices for Claude and Gemini, reported by OpenRouter, n/a for Ollama and other endpoints). Token estimates calibrate themselves from the billed usage (per provider and model, saved in `llm_token_calibration.json`; `TOKEN_RATIO_CALIBRATION=off` disables it). `main.py` / `utils/call_llm.py` check the SDK and credentials of the configured provider before any LLM call.
...
4. Generate a complete codebase tutorial by running the main script:
    ...
    - `--thinking-override` - Per-node overrides, e.g. `--thinking-override write_chapters=high identify_abstractions=xhigh`. Nodes: `filter_files`, `extract_facts`, `map_abstractions`, `reduce_abstractions`, `identify_abstractions`, `analyze_relationships`, `order_chapters`, `write_chapters`, `chapter_summary`, `group_modules`, `translate_strings`.
    ...
    - `--mkdocs` - Format output for MkDocs Material (adds YAML frontmatter & nav snippet).
      ...
      - Section index landing page (`api/index.md`) with grouped module table and one-line module descriptions; in `api-reference` mode also an architecture overview (sections and their dependencies) and a module dependency graph; grouping and descriptions come from the grouping call, the arrows from the source-verified `facts.json` (from the grouping call only for modules whose facts failed). Tutorial/advanced/SDK landing pages show the project summary and relationship diagram.
      - `api-reference` mode also writes `facts.json` (published at `api/facts.json`): every module's symbols, dependencies, config keys and errors, each verified against a quoted line of the source by language-agnostic checks, with line numbers. Lines that look like imports or declarations but no fact reports get one follow-up question, and imports through workspace package names, module paths and path aliases are resolved from the project's manifests (package.json, tsconfig.json, go.mod, Cargo.toml, …). The module dependency graph is drawn from these verified edges, and other tools or AI agents can read the file as ground truth.
...
3. Thiết lập LLM bằng cách sao chép `.env.sample` thành `.env` và cung cấp thông tin xác thực. Theo mặc định, bạn có thể sử dụng khóa API AI Studio cho Gemini bằng cách cài đặt biến môi trường `GEMINI_API_KEY` (hoặc `GEMINI_PROJECT_ID` cho Vertex AI). Nếu bạn muốn sử dụng LLM khác, bạn có thể thiết lập biến `LLM_PROVIDER` (ví dụ: `OPENROUTER`), và sau đó thiết lập model, url và khóa API (ví dụ: `OPENROUTER_MODEL`, `OPENROUTER_BASE_URL`,`OPENROUTER_API_KEY`). Nếu dùng Ollama, thiết lập `LLM_PROVIDER=OLLAMA` và base url (ví dụ: `OLLAMA_BASE_URL=http://localhost:11434`) — có thể bỏ qua API key.
   ...
   **Suy nghĩ trên mọi nhà cung cấp:** `--thinking-profile` (mặc định `auto` = `balanced` với Anthropic, Gemini và OpenRouter) được ánh xạ theo từng model:
   ...
   Mỗi lệnh gọi LLM in một dòng sử dụng (đầu vào bị tính phí so với ước tính, đầu ra kèm suy nghĩ, token từ cache, chi phí), các bước chạy theo lô in tổng phụ, và mỗi lần chạy kết thúc bằng tổng kết token/chi phí theo nhà cung cấp và theo từng bước (chi phí ước tính theo bảng giá với Claude và Gemini, do OpenRouter trả về, không rõ với Ollama và endpoint khác). Ước tính token tự hiệu chỉnh theo số liệu bị tính phí (theo nhà cung cấp và model, lưu trong `llm_token_calibration.json`; đặt `TOKEN_RATIO_CALIBRATION=off` để tắt). `main.py` / `utils/call_llm.py` kiểm tra SDK và thông tin đăng nhập của nhà cung cấp trước mọi lệnh gọi LLM.
...
4. Tạo một bản hướng dẫn toàn diện về mã nguồn bằng cách chạy tập lệnh chính:
    ...
    - `--thinking-override` - Ghi đè theo từng node, ví dụ `--thinking-override write_chapters=high identify_abstractions=xhigh`. Các node: `filter_files`, `extract_facts`, `map_abstractions`, `reduce_abstractions`, `identify_abstractions`, `analyze_relationships`, `order_chapters`, `write_chapters`, `chapter_summary`, `group_modules`, `translate_strings`.
    ...
    - `--mkdocs` - Định dạng đầu ra cho MkDocs Material (thêm YAML frontmatter & nav snippet).
      ...
      - Trang chỉ mục nhóm module (`api/index.md`) với bảng phân loại và mô tả ngắn cho từng module; ở chế độ `api-reference` có thêm tổng quan kiến trúc (các nhóm và phụ thuộc giữa chúng) cùng sơ đồ phụ thuộc giữa các module; phân nhóm và mô tả lấy từ lần gọi phân nhóm, còn các mũi tên lấy từ `facts.json` đã kiểm chứng với mã nguồn (chỉ lấy từ lần gọi phân nhóm cho những module không trích xuất được dữ kiện). Trang chỉ mục của chế độ tutorial/advanced/SDK hiển thị tóm tắt dự án và sơ đồ quan hệ.
      - Chế độ `api-reference` còn ghi `facts.json` (xuất bản tại `api/facts.json`): ký hiệu, phụ thuộc, khóa cấu hình và lỗi của từng module, mỗi mục đều được đối chiếu với một dòng trích từ mã nguồn bằng cách kiểm tra không phụ thuộc ngôn ngữ, kèm số dòng. Những dòng trông như import hoặc khai báo nhưng chưa có dữ kiện nào sẽ được hỏi lại một lần, và các import qua tên gói workspace, đường dẫn module hay bí danh đường dẫn được phân giải từ các tệp manifest của dự án (package.json, tsconfig.json, go.mod, Cargo.toml, …). Đồ thị phụ thuộc module được vẽ từ các cạnh đã kiểm chứng này, và các công cụ khác hoặc AI agent có thể đọc tệp này như nguồn sự thật.
