...
5. **ALWAYS forward `thinking_level` and `step`** to `call_llm()` — resolve the level per call site with `resolve_thinking_level(shared, "<node_key>")` (`utils/thinking.py`), never `shared["thinking_level"]` directly, and pass the same key as `step="<node_key>"` so usage is attributed to that step (per-call usage line, step subtotals, end-of-run table). A new LLM call site needs a NODE key in `NODE_KEYS`, `_PROFILES`, and `MODE_NODES`
6. **ALWAYS use `self.cur_retry`** for cache-on-retry pattern — `call_llm(prompt, use_cache=(use_cache and self.cur_retry == 0), thinking_level=thinking_level, step="<node_key>")`
...
- **LLM settings live at repository level** (Settings → Secrets and variables → Actions → *Repository*): `LLM_PROVIDER`, `OPENROUTER_BASE_URL`, `OPENROUTER_MODEL`, `ANTHROPIC_MODEL` as variables (preferred) or secrets — the workflow reads `vars.X || secrets.X`; `OPENROUTER_API_KEY` / `ANTHROPIC_API_KEY` as secrets only. Never put them in the `docs-deploy` environment: environment-scoped values are visible only to the `approve` job, not to `build-and-deploy` (which deliberately does not use the environment, or it would need a second approval). The `Check LLM settings` step fails the job in seconds when the provider or its key is missing.
