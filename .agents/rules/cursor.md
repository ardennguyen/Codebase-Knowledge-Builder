...
4. **ALWAYS pass 3 args to `log_token_estimation`** — `(node_name, prompt, max_tokens)` not just `(prompt)`
5. **ALWAYS forward `thinking_level`** to `call_llm()` — it's in shared store
6. **ALWAYS use `self.cur_retry`** for cache-on-retry pattern — `call_llm(prompt, use_cache=(use_cache and self.cur_retry == 0))`
...
```bash
python -c "import ast; [ast.parse(open(f).read()) for f in ['main.py','flow.py','nodes.py','utils/call_llm.py','utils/crawl_local_files.py','utils/crawl_github_files.py','utils/token_utils.py']]; print('All files pass syntax check')"
```
